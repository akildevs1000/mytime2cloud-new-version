...
const LiveInsightSidebar = () => {
    ...
    return (
        <>
            {/* Header */}
            <h3 className="text-lg font-bold text-white flex items-center gap-2 sticky top-0 bg-transparent pb-2">
            ...
            <div className="grid grid-cols-2 gap-3">
                {/* Weekly Off Card */}
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col justify-between h-28 relative overflow-hidden group hover:bg-white/10 transition-all">
                    <Palmtree className="absolute -top-2 -right-2 w-16 h-16 opacity-10 group-hover:opacity-20 transition-opacity text-white" />
                ...
                {/* Grace Period Card */}
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col justify-between h-28 relative overflow-hidden group hover:bg-white/10 transition-all">
                    <Timer className="absolute -top-2 -right-2 w-16 h-16 opacity-10 group-hover:opacity-20 transition-opacity text-white" />
                    <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Grace Period</span>
                    <div>
                        <span className="text-2xl font-bold text-white">15 <span className="text-sm font-normal text-slate-400">mins</span></span>
                        <p className="text-xs text-orange-400 mt-1">Late mark enabled</p>
                    </div>
                </div>

                {/* Overtime Card */}
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col justify-between h-28 col-span-2 relative overflow-hidden group hover:bg-white/10 transition-all">
                    <CreditCard className="absolute -top-2 -right-2 w-16 h-16 opacity-10 group-hover:opacity-20 transition-opacity text-white" />
                    ...
                    <div className="flex gap-8 mt-2">
                        <div>
                            <span className="text-2xl font-bold text-white">Pre</span>
                            <p className="text-xs text-slate-400">Before 09:00</p>
                        </div>
                        <div className="w-px bg-white/10 h-full" />
                        <div>
                            <span className="text-2xl font-bold text-white">Post</span>
            ...
            <div className="flex flex-col gap-4 mt-2">
                <div className="flex justify-between items-end">
                    <h4 className="text-sm font-bold text-white">Weekly Schedule Preview</h4>
                ...
                <div className="space-y-3 bg-black/20 p-4 rounded-xl border border-white/5">
                    ...
                    {scheduleData.map((item) => (
                        <div key={item.day} className={`flex items-center gap-3 group ${item.type === 'off' ? 'opacity-40' : ''}`}>
                            <span className={`text-xs font-medium w-7 ${item.type === 'half' ? 'text-blue-400' : 'text-white'}`}>
                                {item.day}
                            </span>
