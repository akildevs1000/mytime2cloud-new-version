...
const SubDepartment = () => {
    ...
    const endpoint = "/subdepartment"; // same as your Vue `endpoint: "subdepartment"`
    ...
    const fetchSubDepartments = async () => {
        try {
            ...
            const data = await getSubDepartments(params);
    ...
    const handleDelete = async (item) => {
        const ok = window.confirm(
            "Are you sure you wish to delete this subdepartment? This action cannot be undone."
        );
        if (!ok) return;
        ...
        try {
            ...
            setMessage(data.message || "Sub Departments deleted successfully.");
        ...
        try {
            if (editedItem.id) {
                ...
                if (!data.status && data.errors) {
                    ...
                    setMessage(data.message || "Sub Departments updated successfully.");
                    setDialogOpen(false);
                    fetchSubDepartments();
                }
            } else {
                ...
                if (!data.status && data.errors) {
                    ...
                    setMessage(data.message || "Sub Departments created successfully.");
                    setDialogOpen(false);
            ...
            console.error(err);
            setError("Failed to save subdepartment.");
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="text-base font-semibold">Sub Departments
                    ...
                    <p className="text-xs text-muted-foreground">
                        Manage subdepartment names used across the system.
                    </p>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="max-w-sm">
                    <DialogHeader>
                        <DialogTitle>
                            {editedItem.id ? "Edit Sub Departments" : "New Sub Departments"}
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-3">
                        <div>
                            <label className="block text-xs font-medium mb-1">
                                Sub Departments Name <span className="text-red-500">*</span>
                            </label>
                            <Input
                                ...
                                placeholder="Sub Departments Name"
                                className="text-sm"
                            />
                            {formErrors?.name && (
