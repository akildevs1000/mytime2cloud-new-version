  ...
  const fetchEmployees = useCallback(async (page, perPage) => {
    ...
    try {
      ...
      if (result && Array.isArray(result.data)) {
        ...
        setSelectedItem(result.data[0] || null); // Select the first employee by default
        return; // Success, exit
      } else {
    ...
    return (
      ...
      >
        <img
          ...
          src={`https://placehold.co/40x40/6946dd/ffffff?text=${employee.name.charAt(0)}`}
          onError={(e) => { e.target.onerror = null; e.target.src = `https://placehold.co/40x40/6946dd/ffffff?text=${employee.name.charAt(0)}`; }}
        />
        <div>
    ...
    <>
      <div className="flex flex-1 gap-6">
        ...
        <div className="flex-1">
          <DeviceEdit selectedItem={selectedItem} />
        </div>
      </div>
    </>
  );
